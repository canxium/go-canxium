...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	for i, result := range results {
		blocks[i] = types.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles).WithWithdrawals(result.Withdrawals).WithProposal(result.Proposal)
		log.Info("Prepared block for insertion", "number", result.Header.Number, "hash", result.Header.Hash(), "txs", len(result.Transactions), "uncles", len(result.Uncles), "withdrawals", len(result.Withdrawals), "have proposals", result.Proposal != nil)
	}
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	block := types.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles).WithWithdrawals(result.Withdrawals).WithProposal(result.Proposal)
