...
var errEthashStopped = errors.New("canxium stopped")
...
func (api *EthAPI) SubmitWork(nonce types.BlockNonce, hash, digest common.Hash) bool {
	if api.canxium.remote == nil {
		return false
	}
...
func (api *EthAPI) SubmitHashrate(rate hexutil.Uint64, id common.Hash) bool {
	...
	select {
	case api.canxium.remote.submitRateCh <- &hashrate{done: done, rate: uint64(rate), id: id}:
	case <-api.canxium.remote.exitCh:
		return false
	}
