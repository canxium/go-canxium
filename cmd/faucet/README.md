...
The `faucet` will use the `les` protocol to join the configured Calcium network and will store its data in `$HOME/.faucet` (currently not configurable).

## Funding

To be able to distribute funds, the `faucet` needs access to an already funded Calcium account. This can be configured via:

- `-account.json` is a path to the Calcium account's JSON key file
