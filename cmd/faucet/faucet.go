...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Calcium protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Calcium and the faucet")
...
type request struct {
	...
	Account common.Address     `json:"account"` // Calcium address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Calcium light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Calcium protocol stack
	client *ethclient.Client   // Client connection to the Calcium chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Calcium light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Calcium service: %w", err)
	}
...
// close terminates the Calcium connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the Calcium address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
...
// the uniqueness identifier (user id/username), username, avatar URL and Calcium address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Calcium address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Calcium address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Calcium address to fund on
// success.
...
// returning the username, avatar URL and Calcium address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Calcium address and profile URL.
	//
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No Calcium address found to fund. Please check the post URL and verify that it can be viewed publicly.")
	}
	var avatar string
...
// authNoAuth tries to interpret a faucet request as a plain Calcium address,
